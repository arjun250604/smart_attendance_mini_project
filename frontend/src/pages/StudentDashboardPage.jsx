...
function TabQRScan() {
  ...
  const startScan = () => {
    ...
    setResult(null)
    setMsg('')
    // Simulate camera scan after 3s
    timerRef.current = setTimeout(() => {
      const code = `SA-CS312-${Math.random().toString(36).slice(2,7).toUpperCase()}`
      setCode(code)
  ...
  const handleScanResult = useCallback((code) => {
    ...
    // Simulate success
    setResult('success')
    setMsg(`Attendance marked for CS312 — Database Systems`)
    setHistory(prev => [
      { id: Date.now(), course: 'CS312', time: 'Just now', status: 'success' },
      ...prev,
    ])
  }, [])
  ...
  const handleManualSubmit = (e) => {
    ...
    const valid = manualCode.startsWith('SA-')
    setResult(valid ? 'success' : 'error')
    setMsg(valid
      ? `Attendance marked via manual code for ${manualCode.split('-')[1]}`
      : 'Invalid session code. Please check and try again.')
    if (valid) {
      setHistory(prev => [
        { id: Date.now(), course: manualCode.split('-')[1] || '—', time: 'Just now', status: 'success' },
        ...prev,
      ])
    }
    setManual('')
        ...
        <div className="sd-card sd-scanner-card">
          ...
          <form id="manual-code-form" onSubmit={handleManualSubmit} className="sd-manual-form">
            <input
              ...
              className="sd-input"
              placeholder="e.g. SA-CS101-ABCD1"
              value={manualCode}
